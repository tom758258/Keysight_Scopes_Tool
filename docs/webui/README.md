# Scopes Tool WebUI

Skeleton package reserved for a future browser-based Scopes Tool interface.

Distribution: `scopes-tool`

Import package: `scopes_tool_webui`

Current status:

- Root package metadata and this import package exist.
- No user-facing WebUI runtime is implemented.
- No CLI workflow is owned by this package.
- Future WebUI behavior should call Core APIs and link to root contracts instead
  of duplicating CLI documentation.

## Docs

The current public WebUI documentation is this README and the package
changelog. Runtime UI documentation should be added when a user-facing WebUI
exists.
