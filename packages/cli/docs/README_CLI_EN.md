...
Package-local command guide for `keysight-scopes` and
`python -m keysight_scope_cli.cli`. This page preserves the detailed command
reference that previously lived in the repository root README.
...
`keysight-scopes`; examples below also use the equivalent module form,
