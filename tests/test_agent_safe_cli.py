...
def test_measure_simulate_json_reports_measurement_fields(capsys):
    ...
    assert result["raw_value"] == "2.500000E-01"
...
def test_capture_simulate_json_reports_files_and_summaries(capsys, tmp_path):
    ...
    assert cli.main(["capture", "--simulate", "--json", "--channel", "1", "--csv", str(csv_path)]) == 0
    ...
    assert result["requested_points"] == 1000
    assert result["actual_points"] == 1000
    assert result["captures"][0]["channel"] == 1
    assert "raw_samples" not in result["captures"][0]
    assert result["captures"][0]["preamble"]["points"] == 1000
...
def test_capture_batch_simulate_json_reports_manifest_and_entries(capsys, tmp_path):
    ...
    assert cli.main(["capture-batch", "--simulate", "--json", "--channel", "1", "--count", "2", "--output-dir", str(output_dir)]) == 0
    ...
    assert result["captures"][0]["actual_points"] == {"CH1": 1000}
